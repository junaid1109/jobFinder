...
<script>
    ...
    const config = {
                    apiKey: "AIzaSyClK7zzqaB79QACRUwFWuu-8kxn--UGN20",
                    authDomain: "notension-a677f.firebaseapp.com",
                    projectId: "notension-a677f",
                    storageBucket: "notension-a677f.appspot.com",
                    messagingSenderId: "883182190442",
                    appId: "1:883182190442:web:adaf21d4a53aa0bca3af13",
                    measurementId: "G-RK7G9TX886"
            };
    ...
    export default {
        ...
        methods: {
                    ...
                    showOTPLogin() {
                        ...
                        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                        const phoneRegex = /^[0-9]{11}$/; // Modified to allow for 11 digits
                     
                        const inputValue = "+971552440350";

                        firebase.initializeApp(config);

                        setTimeout(function() {

                            window.recaptchaVerifier = new firebase.auth.RecaptchaVerifier('recaptcha-containerNew', {
                                'size': 'visible',
                                'callback': (response) => {
                                    // Callback logic (you can leave it empty if not needed)
                                }
                            });
                            ...
                            firebase.auth().signInWithPhoneNumber(inputValue, appVerifier)
                                .then((confirmationResult) => {
                                    // Hide reCAPTCHA widget after successful sign-in
                    ...
                    verifyOtp() {
                                ...
                                firebase.auth().signInWithCredential(credential)
                                    .then(() => {
                                        console.log("Successfully code Matched")
                                        // this.postJob();
                                    })
                                    .catch(error => {
